...
when pli.M_FAMILY =  2 and gin.M_INSTR_TYPE =  0 then 'IRSWP'
...
when pli.M_FAMILY in (32,16384) then rtrim(fcm.M_COMMENT0) 
else null end OBS,
case
...
when pli.M_FAMILY in (32,16384) then rtrim(fcm.M_COMMENT1) 
...
when pli.M_FAMILY in (32) then
   case 
   when fcm.M_INS_MODE = 0 then coalesce(rtrim(fccgencrvfcm.M_LABEL),rtrim(fccgencrvicm.M_LABEL),rtrim(fccgencrvind.M_IND_LAB))
   when fcm.M_INS_MODE = 1 then coalesce(rtrim(fcsgencrvfcm.M_LABEL),rtrim(fcsgencrvicm.M_LABEL),rtrim(fcsgencrvind.M_IND_LAB))
   else null end
--when pli.M_FAMILY in (16384) then coalesce(fccgenivw.M_CRVCAL0, ofcfccgenivw.M_CRVCAL0, ofcfcsgenivw.M_CRVCAL0)
else null end CRVFWD0,
case
...
coalesce(fcsgencrv.M_REFERENCE,fccgencrv.M_REFERENCE) CRVUID0,
case
when pli.M_FAMILY in (2) then
   case 
...
left join CMK_SCCF_DBF   fcsgencrv  on to_char(fcsgenivw.M_ICMUID) = trim(substr(fcsgencrv.M_KEY,1,10)) and fcsgencrv.M__DATE_ = pc.M_DATE and rtrim(fcsgencrv.M__ALIAS_) = 'RT'
left join CMG_GRPI_DBF   fcsgengrp  on fcsgencrv.M_OBJ_PIL = fcsgengrp.M_GROUP and fcsgengrp.M_GTYPE = 512 and fcsgencrv.M__DATE_ = fcsgengrp.M__DATE_ and fcsgencrv.M__ALIAS_ = fcsgengrp.M__ALIAS_
left join CM_FUT_DBF   fcsgencrvfcm on fcsgengrp.M_FUTURE = fcsgencrvfcm.M_REFERENCE
left join CMC_MGEN_DBF fcsgencrvgen on fcsgengrp.M_INSTR_GEN = fcsgencrvgen.M_REFERENCE and fcsgengrp.M_GEN_MODE = 1
left join CM_INDEX_DBF fcsgencrvicm on fcsgengrp.M_INDEX = fcsgencrvicm.M_REFERENCE and fcsgengrp.M_GEN_MODE = 2
left join RT_INDEX_DBF fcsgencrvind on fcsgencrvgen.M_INDEX = fcsgencrvind.M_INDEX
...
left join CMK_SCCF_DBF   fccgencrv  on to_char(fccgenivw.M_ICMUID) = trim(substr(fccgencrv.M_KEY,1,10)) and fccgencrv.M__DATE_ = pc.M_DATE and rtrim(fccgencrv.M__ALIAS_) = 'RT'
left join CMG_GRPI_DBF   fccgengrp  on fccgencrv.M_OBJ_PIL = fccgengrp.M_GROUP and fccgengrp.M_GTYPE = 512 and fccgencrv.M__DATE_ = fccgengrp.M__DATE_ and fccgencrv.M__ALIAS_ = fccgengrp.M__ALIAS_
left join CM_FUT_DBF   fccgencrvfcm on fccgengrp.M_FUTURE = fccgencrvfcm.M_REFERENCE
left join CMC_MGEN_DBF fccgencrvgen on fccgengrp.M_INSTR_GEN = fccgencrvgen.M_REFERENCE and fccgengrp.M_GEN_MODE = 1
left join CM_INDEX_DBF fccgencrvicm on fccgengrp.M_INDEX = fccgencrvicm.M_REFERENCE and fccgengrp.M_GEN_MODE = 2
left join RT_INDEX_DBF fccgencrvind on fccgencrvgen.M_INDEX = fccgencrvind.M_INDEX
-- CM Option on Future
