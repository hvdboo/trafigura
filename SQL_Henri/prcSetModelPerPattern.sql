...
rtrim(asgh.M_LABEL) MODASG, 
rtrim(asgb.M_INST) INSTRUM, rtrim(asgb.M_MDL) MODEL, 
to_char(asgh.M__DATE_,'YYYY-MM-DD') DAT,
reg.CLASS_ID CLA, reg.CLASS_INST REG, pat.M_PAT_ID PAT, pat.M_PRF_ID LAY
...
left join TRAF_INSTRUM ins on reg.CLASS_INST = ins.REG
left join COMMDL_RTGB_DBF asgb on ins.INSTRUM = asgb.M_INST
left join COMMDL_RTGH_DBF asgh on asgb.M__INDEX_ = asgh.M__INDEX_
left join TRN_PC_DBF pc on asgh.M__DATE_ = pc.M_DATE
where 
asgh.M__DATE_ = pc.M_DATE and
asgh.M_LABEL in ('COM_MDL_ASG_1', 'COM_MDL_ASG_1_1') and
...
100, 101, 102, 103, 113, 130, 131, 136, 154,
2661,
2666,
2670,
2711,
...
order by CLASS, REGISTRY, PATTERN, MODASG, INSTRUM