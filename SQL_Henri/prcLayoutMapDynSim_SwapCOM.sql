...
' ',
max(case when pos.M_YCOORD = 0 and pos.M_INDEX = 0 then rtrim(mapDyn.M_TGTCOL) else null end) DYNPL0,
max(case when pos.M_YCOORD = 1 and pos.M_INDEX = 0 then rtrim(mapDyn.M_TGTCOL) else null end) DYNPL1a,
max(case when pos.M_YCOORD = 1 and pos.M_INDEX = 1 then rtrim(mapDyn.M_TGTCOL) else null end) DYNPL1b,
max(case when pos.M_YCOORD = 1 and pos.M_INDEX = 2 then rtrim(mapDyn.M_TGTCOL) else null end) DYNPL1c,
' ',
max(case when pos.M_YCOORD = 0 and pos.M_INDEX = 0 then rtrim(mapSim.M_TGTCOL) else null end) SIM0,
max(case when pos.M_YCOORD = 1 and pos.M_INDEX = 0 then rtrim(mapSim.M_TGTCOL) else null end) SIMa,
max(case when pos.M_YCOORD = 1 and pos.M_INDEX = 1 then rtrim(mapSim.M_TGTCOL) else null end) SIM1b,
max(case when pos.M_YCOORD = 1 and pos.M_INDEX = 2 then rtrim(mapSim.M_TGTCOL) else null end) SIM1c
from NP_POS_DBF pos 
...
left join UDTB279_DBF regl on leg.M_REG_ID = regl.M_REG_FGT
left join UDTB279_DBF regp on rtrim(pat.M_REG_DESC) = rtrim(regp.M_REG_DES)
left join DCF_OBJ_DBF npd on leg.M_PAT_ID = npd.M_ITEM_ID
left join UDTB293_DBF buzl on (trim(pos.M_LABEL) = buzl.M_BUZFLD and leg.M_REG_ID = buzl.M_BUZCLA)
left join UDTB293_DBF buzp on (trim(pos.M_LABEL) = buzp.M_BUZFLD and regp.M_REG_FGT = buzp.M_BUZCLA)
...
left join UDTB291_DBF mapDyn on (rtrim(pos.M_LABEL) = rtrim(mapDyn.M_SRCCOL) and (mapDyn.M_SRCTBL = '130' or  mapDyn.M_SRCTBL = '1'))
left join UDTB291_DBF mapSim on (rtrim(pos.M_LABEL) = rtrim(mapSim.M_SRCCOL) and (mapSim.M_SRCTBL = '130' or  mapSim.M_SRCTBL = '2'))

where pos.M_STRATID in (100)
