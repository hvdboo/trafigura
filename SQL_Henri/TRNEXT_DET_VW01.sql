drop view MUREX_MX_OWNER.TRNEXT_DET_VW01;
create view MUREX_MX_OWNER.TRNEXT_DET_VW01
(
PCDAT,
TRNDAT,
SRC,
...
-- to_char(pc.M_DATE,'YYYYMMDD') TRNDAT,
-- Identifiers
...
when trn.M_COMMENT_BS = 'B' then
        ...
        when ble.M_DSP_LABEL in ('PTE','TIC') then
                case 
...
when trn.M_COMMENT_BS = 'S' then
        ...
        when sle.M_DSP_LABEL in ('PTE','TIC') then
                case 
...
-- !! Adapt contextually !! --
case when trn.M_BRW_ODPL = '16-JUL-19' then 'CASH' else
case when trn.M_BRW_ODPL = '11-OCT-19' then '3M' else
...
case 
when trn.M_TRN_GTYPE in (101) then to_number('0.01')
when trn.M_TRN_GTYPE in (84,103,113,131,146) then trn.M_BRW_RTE1 
else 0 end PRM,
-- Price
case  
when trn.M_TRN_GTYPE in (101) then to_number('0.01')
when trn.M_TRN_GTYPE in (77) then fxs.M_XPFWPRC 
else trn.M_BRW_RTE1 end PRC1, 
trn.M_BRW_MRG1 MRG1,
trn.M_BRW_RTE2 PRC2,
trn.M_BRW_MRG2 MRG2,
-- Direction
...
left join MUREX_MX_OWNER.TRN_PFLD_DBF bpf on trn.M_BPFOLIO = bpf.M_LABEL
left join MUREX_MX_OWNER.TRN_PFLD_DBF spf on trn.M_SPFOLIO = spf.M_LABEL
...
left join MUREX_MX_OWNER.USAGE_DBF usg on trn.M_USAGE = usg.M_REFERENCE
...
left join MUREX_MX_OWNER.RT_INDEX_DBF ind1 on plkey.M_INDEX = ind1.M_INDEX
left join MUREX_MX_OWNER.CM_INDEX_DBF cmi1 on ind1.M_COM_IND = cmi1.M_REFERENCE
left join MUREX_MX_OWNER.CMC_QUOT_DBF qot1 on plkey.M_QUOT = qot1.M_REFERENCE
left join MUREX_MX_OWNER.CM_MKT_DBF pub1 on qot1.M_PUBLI= pub1.M_REFERENCE
left join MUREX_MX_OWNER.CM_INDEX_DBF cmu1 on plkey.M_UNDL = cmu1.M_REFERENCE
left join MUREX_MX_OWNER.RT_INDEX_DBF ind2 on trim(substr(trn.M_MKT_INDEX,18,15)) = trim(ind2.M_INDEX)
left join MUREX_MX_OWNER.CM_INDEX_DBF cmi2 on ind2.M_COM_IND = cmi2.M_REFERENCE
left join MUREX_MX_OWNER.CMC_QUOT_DBF qot2 on ind2.M_COM_QUOT = qot2.M_REFERENCE
left join MUREX_MX_OWNER.CM_MKT_DBF pub2 on qot2.M_PUBLI= pub2.M_REFERENCE
left join MUREX_MX_OWNER.RT_INDEX_DBF und2 on rtrim(ind2.M_UNDRL) = rtrim(und2.M_INDEX)
left join MUREX_MX_OWNER.CM_INDEX_DBF cmu2 on und2.M_COM_IND = cmu2.M_REFERENCE
left join MUREX_MX_OWNER.CM_UNIT_DBF unq on qot1.M_UNIT = unq.M_REFERENCE
...
left join MUREX_MX_OWNER.RT_GROUP_DBF grp on (to_char(fut.M_REFERENCE) = trim(substr(grp.M_GRP_DESC,1,10)) and to_char(qot1.M_REFERENCE) = trim(substr(grp.M_GRP_DESC,11,15)))
...
left join MUREX_MX_OWNER.CM_MKTSR_DBF ghsr1 on trim(substr(gen.M_FORMULA0,2,10)) = to_char(ghsr1.M_SERIE)
...
left join MUREX_MX_OWNER.TRN_BROKER_DBF brk0 on (trn.M_NB = brk0.M_NB and brk0.M_LINE = 0)
left join MUREX_MX_OWNER.TRN_BROKER_DBF brk1 on (trn.M_NB = brk1.M_NB and brk1.M_LINE = 1)
left join MUREX_MX_OWNER.TRN_BROKER_DBF brk2 on (trn.M_NB = brk2.M_NB and brk2.M_LINE = 2)
left join MUREX_MX_OWNER.TRN_BROKER_DBF brk3 on (trn.M_NB = brk3.M_NB and brk3.M_LINE = 3)
left join MUREX_MX_OWNER.TRN_PFLD_DBF brkpfl0 on brk0.M_SRC_PFOLIO = brkpfl0.M_REF
left join MUREX_MX_OWNER.TRN_PFLD_DBF brkpfl1 on brk1.M_SRC_PFOLIO = brkpfl1.M_REF
left join MUREX_MX_OWNER.TRN_PFLD_DBF brkpfl2 on brk2.M_SRC_PFOLIO = brkpfl2.M_REF
left join MUREX_MX_OWNER.TRN_PFLD_DBF brkpfl3 on brk3.M_SRC_PFOLIO = brkpfl3.M_REF
left join MUREX_MX_OWNER.TRN_CPDF_DBF brkctp0 on brk0.M_CNTRP = brkctp0.M_ID
left join MUREX_MX_OWNER.TRN_CPDF_DBF brkctp1 on brk1.M_CNTRP = brkctp1.M_ID
left join MUREX_MX_OWNER.TRN_CPDF_DBF brkctp2 on brk2.M_CNTRP = brkctp2.M_ID
left join MUREX_MX_OWNER.TRN_CPDF_DBF brkctp3 on brk3.M_CNTRP = brkctp3.M_ID
where 1 = 1
and trn.M_PURPOSE <> 'MeHzv70053'
and trn.M_BINTERNAL <> trn.M_SINTERNAL
and trn.M_TRN_STATUS <> 'DEAD'
and coalesce(ble.M_DSP_LABEL,sle.M_DSP_LABEL) in ('TTS','TEZ')
