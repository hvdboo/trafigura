...
left join MUREX_DM_OWNER.TEC_MXREG regl on leg.M_REG_ID = regl.CLASS_INST
left join MUREX_DM_OWNER.TEC_MXREG regp on rtrim(pat.M_REG_DESC) = rtrim(regp.LAB)
left join MUREX_MX_OWNER.DCF_OBJ_DBF npd on leg.M_PAT_ID = npd.M_ITEM_ID
left join MUREX_DM_OWNER.TEC_MXBUZ buzl on (trim(pos.M_LABEL) = buzl.FLD and leg.M_REG_ID = buzl.TYP)
left join MUREX_DM_OWNER.TEC_MXBUZ buzp on (trim(pos.M_LABEL) = buzp.FLD and regp.CLASS_INST = buzp.TYP)
...
left join MUREX_DM_OWNER.TEC_MAPDIC mapDyn on (mapDyn.TYP = 1 and (rtrim(pos.M_LABEL) = rtrim(mapDyn.S_COL) and (mapDyn.S_TBL = '101' or  mapDyn.S_TBL = '0')))
left join MUREX_DM_OWNER.TEC_MAPDIC mapSim on (mapSim.TYP = 2 and (rtrim(pos.M_LABEL) = rtrim(mapSim.S_COL) and (mapSim.S_TBL = '101' or  mapSim.S_TBL = '0')))

where pos.M_STRATID in (95)
