...
left join TRAF_MAPDIC mapDyn on (mapDyn.TYP = 1 and (rtrim(pos.M_LABEL) = rtrim(mapDyn.S_COL) and (mapDyn.S_TBL = '134' or  mapDyn.S_TBL = '0')))
left join TRAF_MAPDIC mapSim on (mapSim.TYP = 2 and (rtrim(pos.M_LABEL) = rtrim(mapSim.S_COL) and (mapSim.S_TBL = '134' or  mapSim.S_TBL = '0')))
where pos.M_STRATID in (106)
