...
max(case when pos.M_YCOORD = 1 and pos.M_INDEX = 0 and pos.M_RFIXED <> 2560 then rtrim(mapSim.T_COL) else null end)||max(case pos.M_RFIXED when 2560 then ' (' else '' end) || 
max(case when pos.M_YCOORD = 1 and pos.M_INDEX = 0 and pos.M_RFIXED =  2560 then rtrim(mapSim.T_COL) else null end)||max(case pos.M_RFIXED when 2560 then ')'  else '' end) SIM1,
max(case when pos.M_YCOORD = 1 and pos.M_INDEX = 1 then rtrim(mapSim.T_COL) else null end) SIM2,
max(case when pos.M_YCOORD = 1 and pos.M_INDEX = 2 then rtrim(mapSim.T_COL) else null end) SIM3
from NP_POS_DBF pos 
...
left join TRAF_MAP mapSim on (mapSim.TYP = 2 and (rtrim(pos.M_LABEL) = rtrim(mapSim.S_COL) and (mapSim.S_TBL = '130' or  mapSim.S_TBL = '0')))
where pos.M_STRATID in (2371) 
