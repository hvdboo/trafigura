...
max(case when pos.M_YCOORD = 1 and pos.M_INDEX = 0 and pos.M_RFIXED <> 2560 then rtrim(pos.M_LABEL) else null end)||max(case pos.M_RFIXED when 2560 then ' (' else '' end) || 
max(case when pos.M_YCOORD = 1 and pos.M_INDEX = 0 and pos.M_RFIXED =  2560 then rtrim(pos.M_LABEL) else null end)||max(case pos.M_RFIXED when 2560 then ')'  else '' end) COL1,
max(case when pos.M_YCOORD = 1 and pos.M_INDEX = 1 then rtrim(pos.M_LABEL) else null end) COL2,
max(case when pos.M_YCOORD = 1 and pos.M_INDEX = 2 then rtrim(pos.M_LABEL) else null end) COL3,
'#',
-- max(case when pos.M_YCOORD = 0 and pos.M_INDEX = 0 then rtrim(mapDyn.T_COL) else null end) DYN0,
max(case when pos.M_YCOORD = 1 and pos.M_INDEX = 0 and pos.M_RFIXED <> 2560 then rtrim(mapDyn.T_COL) else null end)||max(case pos.M_RFIXED when 2560 then ' (' else '' end) || 
max(case when pos.M_YCOORD = 1 and pos.M_INDEX = 0 and pos.M_RFIXED =  2560 then rtrim(mapDyn.T_COL) else null end)||max(case pos.M_RFIXED when 2560 then ')'  else '' end) DYN1,
max(case when pos.M_YCOORD = 1 and pos.M_INDEX = 1 then rtrim(mapDyn.T_COL) else null end) DYN2,
max(case when pos.M_YCOORD = 1 and pos.M_INDEX = 2 then rtrim(mapDyn.T_COL) else null end) DYN3
from NP_POS_DBF pos 
...
left join TRAF_MAP mapDyn on (mapDyn.TYP = 1 and (rtrim(pos.M_LABEL) = rtrim(mapDyn.S_COL) and (mapDyn.S_TBL = to_char(${REG}) or  mapDyn.S_TBL = '0')))
