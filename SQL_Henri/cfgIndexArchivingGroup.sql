...
when  3 then rtrim(cmpub.M_LABEL)
when  4 then rtrim(cmipub.M_LABEL)
...
from RT_GROUP_DBF arc
left join CAL_DEF_DBF cal on arc.M_CALENDAR = cal.M_LABEL
...
left join CM_MKT_DBF cmpub on trim(arc.M_GRP_CATEG) = to_char(cmpub.M_REFERENCE) 
left join CM_MKT_DBF cmfpub on cmfqot.M_PUBLI = cmfpub.M_REFERENCE 
left join CM_MKTSR_DBF cmfhsr on cmfpub.M_REFERENCE = cmfhsr.M_REFERENCE
left join CM_MKT_DBF cmipub on trim(substr(arc.M_GRP_CATEG,1,10)) = to_char(cmipub.M_REFERENCE) and arc.M_GRP_TYPE = 4
left join CM_MKTSR_DBF cmihsr on cmipub.M_REFERENCE = cmihsr.M_REFERENCE
left join SE_MKT2_DBF eqo on trim(substr(arc.M_GRP_CATEG,1,10)) = to_char(eqo.M_SE_OPT_IN) and arc.M_GRP_TYPE = 10
left join CM_MKTSR_DBF eqhsr on arc.M_SERIES = eqhsr.M_REFERENCE and M_PRICE = 2
left join SE_HEAD_DBF sec on trim(substr(arc.M_GRP_DESC,length(rtrim(arc.M_GRP_CATEG))+1,20)) = trim(sec.M_SE_LABEL)
-- where rtrim(arc.M_CALENDAR) = ?
-- where arc.M_GRP_TYPE in (2) and rtrim(sec.M_SE_CATE) = 'Equity'
