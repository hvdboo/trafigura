...
rtrim(mca.M_LABEL) MCA, 
rtrim(mcat.M_LABEL) MCATYP,
...
when rtrim(adc.M_LABEL) = 'FRT'       then 'Freight'
...
when rtrim(adc.M_LABEL) = 'LG LIQUID' then 'Liquified gas'
...
case scop.M_CRIT_TYPE when 1 then rtrim(ctp.M_DSP_LABEL) else null end CTP,
rtrim(substr(mcat.M_LABEL,9,3)) POS,
case scop.M_CRIT_TYPE when 4 then rtrim(ctp.M_DSP_LABEL) else null end LE,
case scop.M_CRIT_TYPE when 5 then rtrim(adc.M_LABEL) else null end ASS,
1 OCC
...
order by MCA, substr(MCATYP,9,3), CRITTYP

) und

group by und.CTP, und.POS, und.LE, und.ASS
