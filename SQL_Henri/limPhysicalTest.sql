select
rtrim(th.M_LABEL) as M_TREE, t.M_HEIGHT,
...
case t.M_D_DATA2 
   when 1 then 'Elt' 
   when 2 then 'Fld' end as M_NOD_TYP,
case t.M_D_DATA2 
   when 1 then rtrim(elt.M_LABEL) 
   when 2 then rtrim(fld.M_LABEL) end as M_NOD_LAB,
rtrim(elt.M_DESC) as M_PHY_DES, 
t.M_REF as M_NOD_REF,
--0.00 as M_LIM_MTM, 0.00 as M_LIM_VAR, 0.00 as M_LIM_STR
...
left outer join CMU_MMST_DBF t1 on (t1.M_FATHER_R = 0        and t1.M_HEIGHT = 1 and rtrim(t1.M_TRE_GROUP) = 'CMU_TREEG' and to_number(t1.M_TRE_NAME) = th.M_REFERENCE)
