...
count(case when rtrim(src.M_LABEL) = 'TRT' then 1 else null end) TRT, 
count(case when rtrim(src.M_LABEL) <> 'TRT' then 1 else null end) OTHER,
count(*) TRADES
...
trn.M_TRN_GTYPE, plin.M_DSP_LABEL, trn.M_BRW_ODPL, trn.M_TRN_EXP, trn.M_BRW_STRK, trn.M_BRW_CP, 
(case when trn.M_COMMENT_BS ='B' then rtrim(trn.M_BPFOLIO) else rtrim(trn.M_SPFOLIO) end)
order by TRN_IE, ENT, CTP_IE, CTP, LO, FGT, INSTRUMENT, EXP, STK, CP, PFL