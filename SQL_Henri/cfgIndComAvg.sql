...
case und.M_RESET
when 0 then rtrim(undpub.M_LABEL)
when 3 then rtrim(undgrp.M_GRP_DESC)  
when 4 then rtrim(undgrp.M_GRP_DESC)
when 6 then rtrim(undpub.M_LABEL) else null end UNDPUB,
rtrim(undqot.M_TRAD_SMB) UNDSYM,
coalesce(rtrim(undpub.M_CALENDAR), rtrim(undgrp.M_CALENDAR)) UNDCAL,
coalesce(rtrim(undqot.M_CURR), rtrim(und.M_CURRENCY))     CUR, 
coalesce(rtrim(undqotuoq.M_LABEL), rtrim(unduoq.M_LABEL)) UOQ,
coalesce(rtrim(undqotuod.M_LABEL), rtrim(unduod.M_LABEL)) UOD,
case und.M_RESET
...
when 3 then null
when 4 then null
when 6 then coalesce(rtrim(undnbypub.M_LABEL), rtrim(undnbyfccpub.M_LABEL), rtrim(undnbyfcspub.M_LABEL)) else null end ICMPUB,
case und.M_RESET
...
case und.M_RESET
when 0 then rtrim(undhsrdfl.M_HSR) 
when 3 then null
when 4 then null
when 6 then coalesce(rtrim(undnbyhsrdfl.M_HSR), rtrim(undnbyfcchsrdfl.M_HSR), rtrim(undnbyfcshsrdfl.M_HSR)) else null end ICMHSR,
case und.M_RESET
when 0 then rtrim(undpub.M_CALENDAR) 
when 3 then null
when 4 then null
when 6 then coalesce(rtrim(undnbypub.M_CALENDAR), rtrim(undnbyfccpub.M_CALENDAR), rtrim(undnbyfcspub.M_CALENDAR)) else null end ICMCAL,
-- ROUNDING
...
left join CM_PHYS_DBF  undicmphy on undicm.M_PHYSICAL = undicmphy.M_REFERENCE
left join CM_LOCAT_DBF undicmloc on undicm.M_LOCATION = undicmloc.M_REFERENCE
...
left join VIW_ICMPUB_DBF undhsrdfl on undpub.M_REFERENCE = undhsrdfl.M_PUBUID and undhsrdfl.M_HSRDFL = 1
left join CM_FUT_DBF   undnbyfcm on (und.M_COM_FUT = undnbyfcm.M_REFERENCE and und.M_COM_NBY_T = 0)
left join RT_INSGN_DBF undnbyfccins on (undnbyfcm.M_CM_INSTR = undnbyfccins.M_GEN_NUM and undnbyfcm.M_INS_MODE = 0)
left join RT_LNGN_DBF  undnbyfccgen on (undnbyfcm.M_CM_INSTR = undnbyfccgen.M_GEN_NUM and undnbyfcm.M_LISTED in (1,2,16,32) and undnbyfcm.M_INS_MODE = 0)
...
left join CM_MKT_DBF   undnbyfccpub on undnbyfccqot.M_PUBLI = undnbyfccpub.M_REFERENCE
left join VIW_ICMPUB_DBF undnbyfcchsrdfl on undnbyfccpub.M_REFERENCE = undnbyfcchsrdfl.M_PUBUID and undnbyfcchsrdfl.M_HSRDFL = 1
left join CMC_MGEN_DBF undnbyfcsgen on (undnbyfcm.M_CM_INSTR = undnbyfcsgen.M_REFERENCE and undnbyfcm.M_LISTED in (1,2,16) and undnbyfcm.M_INS_MODE = 1)
...
left join CM_MKT_DBF   undnbyfcspub on undnbyfcsqot.M_PUBLI = undnbyfcspub.M_REFERENCE
left join VIW_ICMPUB_DBF undnbyfcshsrdfl on undnbyfcspub.M_REFERENCE = undnbyfcshsrdfl.M_PUBUID and undnbyfcshsrdfl.M_HSRDFL = 1
...
left join CM_MKT_DBF   undnbypub on undnbyqot.M_PUBLI = undnbypub.M_REFERENCE
left join VIW_ICMPUB_DBF undnbyhsrdfl on undnbypub.M_REFERENCE = undnbyhsrdfl.M_PUBUID and undnbyhsrdfl.M_HSRDFL = 1
