...
case
when fcm.M_LISTED in (1,2,16,32) then coalesce(fccidt.M_ICMLAB, fcsidt.M_ICMLAB)
when fcm.M_LISTED in (64) then coalesce(ofcfccidt.M_ICMLAB, ofcfcsidt.M_ICMLAB)
else null end ICMLAB,
-- DELIVERY

case
when fcm.M_LISTED in (1,2,16,32) then coalesce(fccidt.M_PHY, fcsidt.M_PHY)
when fcm.M_LISTED in (64) then coalesce(ofcfccidt.M_PHY, ofcfcsidt.M_PHY)
else null end PHY,
case
when fcm.M_LISTED in (1,2,16,32) then coalesce(fccidt.M_LOC, fcsidt.M_LOC)
when fcm.M_LISTED in (64) then coalesce(ofcfccidt.M_LOC, ofcfcsidt.M_LOC)
else null end LOC,
*/
-- HIS
