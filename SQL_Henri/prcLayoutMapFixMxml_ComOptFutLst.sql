...
-- FIX + MxML
max(case when pos.M_YCOORD = 1 and pos.M_INDEX = 0 then rtrim(mapFix.M_TGTVALN) else null end) FIXNUM1a,
max(case when pos.M_YCOORD = 1 and pos.M_INDEX = 0 then rtrim(mapFix.M_TGTCOL) else null end) FIXFLD1a,
...
max(case when pos.M_YCOORD = 1 and pos.M_INDEX = 1 then rtrim(mapFix.M_TGTVALN) else null end) FIXNUM1b,
max(case when pos.M_YCOORD = 1 and pos.M_INDEX = 1 then rtrim(mapFix.M_TGTCOL) else null end) FIXFLD1b,
...
left join MUREX_MX_OWNER.UDTB291_DBF mapMxml on (mapMxml.M__INDEX_ = 7988809 and (rtrim(pos.M_LABEL) = rtrim(mapMxml.M_SRCCOL) and (mapMxml.M_SRCTBL = '101' or  mapMxml.M_SRCTBL = '0')))
left join MUREX_MX_OWNER.UDTB291_DBF mapFix on (mapFix.M__INDEX_ = 8084294 and (rtrim(pos.M_LABEL) = rtrim(mapFix.M_SRCCOL) and (mapFix.M_SRCTBL = '101' or  mapFix.M_SRCTBL = '0')))

where pos.M_STRATID in (2080191)
