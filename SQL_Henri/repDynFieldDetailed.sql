...
case when hor4.M_DUP_LABEL is not null then rtrim(hor4.M_DUP_DESC) else rtrim(dyndic.FLD_DESC) end DES, 
dyndic.FLD_RNG RNG,
coalesce(hor4.M_DUP_TYPE, dyndic.FLD_TYPE) TYP, 
coalesce(hor4.M_DUP_WIDTH, dyndic.FLD_LENGTH) LEN, 
coalesce(hor4.M_DUP_DEC, dyndic.FLD_DEC)  DCM,
...
left join mxdyntbl dyndic on rtrim(fld4.M_FLD_LABEL) = rtrim(dyndic.FLD_LABEL)
