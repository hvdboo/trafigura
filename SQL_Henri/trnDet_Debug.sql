...
rtrim(inisrc.M_CHAR_VAL) PFLSRC,
rtrim(initgt.M_CHAR_VAL) PFLTGT,
...
-- CFST1
case  
when trn.M_TRN_GTYPE in (100,101,102,103,113,130,131,134, 146,154) then to_char(dlv.M_CALC_FST,'YYYY-MM-DD') 
when trn.M_TRN_GTYPE in (76,77,84,92) then to_char(fxs.M_XPFWEXP,'YYYY-MM-DD') 
when trn.M_TRN_GTYPE in (1,2,5) then to_char(loan.M_START_DAT0,'YYYY-MM-DD')
else to_char(trn.M_OPT_FLWFST,'YYYY-MM-DD') end CFST1,
-- CLST1
case 
when trn.M_TRN_GTYPE in (100,101,102,103,113,130,131,134, 146,154) then to_char(dlv.M_CALC_LST,'YYYY-MM-DD')
when trn.M_TRN_GTYPE in (1,2,5) then to_char(loan.M_MATURITY0,'YYYY-MM-DD')
else to_char(trn.M_OPT_FLWLST,'YYYY-MM-DD') end CLST1,
-- CFST2
case when trn.M_BRW_FV2 = 'V' then
case  
when trn.M_TRN_GTYPE in (100,101,102,103,113,130,131,134, 146,154) then to_char(loan.M_START_DAT1,'YYYY-MM-DD')
when trn.M_TRN_GTYPE in (76,77,84,92) then to_char(fxs.M_XPFWEXP,'YYYY-MM-DD')
when trn.M_TRN_GTYPE in (1,2,5) then to_char(loan.M_START_DAT1,'YYYY-MM-DD')
else null end end CFST2,
-- CLST2
case when trn.M_BRW_FV2 = 'V' then
case 
when trn.M_TRN_GTYPE in (130,154) then to_char(loan.M_MATURITY1-1,'YYYY-MM-DD')
when trn.M_TRN_GTYPE in (100,101,102,103,113,134) then to_char(loan.M_MATURITY1,'YYYY-MM-DD')
when trn.M_TRN_GTYPE in (1,2) then to_char(loan.M_MATURITY1,'YYYY-MM-DD')
else null end end CLST2,
-- STL
case when trn.M_TRN_GTYPE in (84) then to_char(trn.M_OPT_FLWFST,'YYYY-MM-DD') else to_char(trn.M_BRW_SDTE,'YYYY-MM-DD') end STL,
-- EXP
to_char(trn.M_TRN_EXP,'YYYY-MM-DD') EXP,
case when trn.M_TRN_GTYPE in (84, 101, 103, 113, 131, 146) then trn.M_BRW_STRK else null end STK,
rtrim(trn.M_BRW_CP) RGT,
round(fxg.M_FXNG_VALUE,2) FXG,
case trn.M_BRW_CP
when 'C' then case when (round(fxg.M_FXNG_VALUE,2) - trn.M_BRW_STRK) > 0 then 'ITM' else 'OTM' end
when 'P' then case when (round(fxg.M_FXNG_VALUE,2) - trn.M_BRW_STRK) < 0 then 'ITM' else 'OTM' end
else null end MON,
case when trn.M_TRN_GTYPE in (77) then fxs.M_XPFWPRC else trn.M_BRW_RTE1 end PRC, 
trn.M_BRW_MRG1 MRG1,
-- case when trn.M_BRW_FV2 = 'V' then trn.M_BRW_RTE2 else null end PRC2,
case when trn.M_BRW_FV2 = 'V' then trn.M_BRW_MRG2 else null end MRG2,
coalesce(fut.M_QTY,1) LOTSIZ,
case 
when trn.M_TRN_GTYPE in (130,131,154) then round(trn.M_BRW_NOM1/und.M_LOTSIZE,4)
else round(trn.M_BRW_NOM1,4) end NOM1,
case when trn.M_TRN_FMLY <> 'COM' then round(trn.M_BRW_NOM2,4) else null end NOM2,
...
left join MUREX_MX_OWNER.NP_IDATA_DBF inisrc on trn.M_NB = inisrc.M_SNAP_ID and rtrim(inisrc.M_LABEL) = 'Portfolio (input)'
left join MUREX_MX_OWNER.NP_IDATA_DBF initgt on trn.M_NB = initgt.M_SNAP_ID and rtrim(initgt.M_LABEL) = 'Portfolio facing'
...
left join MUREX_MX_OWNER.FXNG_DBF fxg on trn.M_NB = fxg.M_TRN_NUMBER

/*-- Where clause depends on scope*/
where
trn.M_PURPOSE <> 'MeHzv70053'
and trn.M_MOP_LAST not in (6,7)
-- and ext.M_EVT_INTID <> '1.220'
-- and trn.M_TRN_DATE > trn.M_TRN_EXP
and trn.M_NB = 17434958

order by GTYP, TRNDAT, TRN
