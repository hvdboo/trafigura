...
-- cnt.LAB CMP_CNTLAB, cnt.ID, cnt.DES CMP_CNTDES,
dlv.LAB DLV, 
phy.LAB PHY, qly.LAB QLY, loc.LAB LOC, trm.LAB TRM, car.LAB CAR,
pub.LAB PUB, 
mrk.SYM SYM,
case mrk.PUB
...
when  18 then rtrim(cme.ProductName)
when  61 then rtrim(cme.ProductName)
...
left join reg.cme cme on (mrk.COD = cme.Clearing and mrk.PUB in (18, 61) and cme.ProductGroup = 'Energy') 
where traf.NAT = 'Outright'
