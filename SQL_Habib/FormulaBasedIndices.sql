...
set pagesize 2048;
select
CAST(T1.M_IND_LAB AS VARCHAR2(36)) as FormulaBasedIndex, 
	case	 when (T1.M_CATEGORY  = 7 ) then 'Formula' end as Category,
 ...
 case 	when  ((T1.M_RESET in (2,3,5))  or ((T1.M_RESET =4) and (T1.M_AVG_UND = 1)))  and (T1.M_CRND_RULE =0 ) then 'None' 
	...
	case when  ((T1.M_RESET in (2,3,5))  or ((T1.M_RESET =4) and (T1.M_AVG_UND = 1)))  and (T1.M_CRND_RULE <>0) then to_char(T1.M_CRND_DEC) else ' ' end as ComputedRndgDec,
       ...
       case when  ((T1.M_RESET = 4) and (T1.M_BSK_MODE = 7)) then cast(replace(replace (T6.M_BUFFER, chr(10), ' '), chr(13), ' ') as VARCHAR2(100))
		else ' ' end as BskFormulaDetails,
...
case    when ((T1.M_RESET in (2,3,5))  or ((T1.M_RESET =4) and (T1.M_AVG_UND = 1)))  and (T1.M_F_SHIFT = 0) then 'No'
        ...
        case    when ((T1.M_F_SHIFT = 1) and ((T1.M_RESET in (2,3,5))  or ((T1.M_RESET =4) and (T1.M_AVG_UND = 1)))  and (T1.M_U_FSHIFT = 0)) then 'First'
                when ((T1.M_F_SHIFT = 1) and (((T1.M_RESET in (2,3,5))  or ((T1.M_RESET =4)) and (T1.M_AVG_UND = 1)))  and  (T1.M_U_FSHIFT= 1)) then 'Last' else ' ' end as Choice1, 
        ...
        case    when (T1.M_L_SHIFT = 1) and ((T1.M_RESET in (2,3,5))  or ((T1.M_RESET =4) and (T1.M_AVG_UND = 1)))    and  (T1.M_U_LSHIFT = 1) then 'First'
                when ((T1.M_L_SHIFT = 1) and ((T1.M_RESET in (2,3,5))  or ((T1.M_RESET =4) and (T1.M_AVG_UND = 1)))    and (T1.M_U_LSHIFT = 0)) then 'Last' else ' ' end as Choice2,
	...
	case 	when ((T1.M_RESET =3)  and (T1.M_BROKEN = 0)) then 'Up front' 
		        when  ((T1.M_RESET =3)  and (T1.M_BROKEN = 1)) then 'In arrears' else ' ' end as StubPeriod ,
