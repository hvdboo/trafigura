...
/*
case INS 
when 'FRT BCI - LCH'      then 'TRADER41'
when 'FRT BCI - SGX'      then 'TRADER42'
when 'FRT BPI - LCH'      then 'TRADER43'
when 'FRT BSI - LCH'      then 'TRADER44'
when 'FRT CS TCA 5 - LCH' then 'TRADER45'
*/
when 'MCEX XX- PTEI'   then 'TRADER50' 
/*
case INS 
when 'FRT BCI - SGX'      then 'TRADER51'
when 'FRT BPI - SGX'      then 'TRADER52'
when 'FRT BSI - SGX'      then 'TRADER53'
when 'FRT CS TCA 5 - SGX' then 'TRADER54'
when 'FRT S TCA 10 - LCH' then 'TRADER55'
*/
...
-- Offset: keep original instrument
max(rtrim(pub.M_LABEL)) PUB,
max(rtrim(fut.M_LABEL)) INS,
max(rtrim(trn.M_BRW_ODPL)) MAT,
to_char(fmat.M_QT_END,'YYYYMMDD') EXP,
-- Offset: switch buy/sell
max(case trn.M_COMMENT_BS
when 'B' then 'Sell' 
when 'S' then 'Buy' else null end) DIR,
