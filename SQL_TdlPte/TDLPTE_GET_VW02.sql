drop view TDLPTE_VW02;

create view TDLPTE_VW02  
(
SYSDAT,
TRN_IE, 
CTP_IE, 
...
case when trn.M_COMMENT_BS ='B' then rtrim(trn.M_BTRADER) else rtrim(trn.M_STRADER) end,
...
rtrim(trn.M_TRN_FMLY)||'/'||rtrim(trn.M_TRN_GRP)||'/'||coalesce(rtrim(trn.M_TRN_TYPE),' '),
rtrim(typo.M_LABEL),
rtrim(plin.M_DSP_LABEL),
rtrim(fut.M_LABEL),
rtrim(coalesce(ind.M_IND_LAB,fxs.M_XPFWGROUP)),
rtrim(coalesce(und.M_LABEL,fund.M_LABEL)),
case 
...
case when trn.M_TRN_FMLY = 'COM' then qot.M_CURR else trn.M_BRW_NOMU1 end,
...
rtrim(futmat.M_LABEL), 
rtrim(optmat.M_LABEL),
-- CFST1
case  
...
else trn.M_BRW_NOM1 end,
trn.M_BRW_NOM2,
ABS(trn.M_BRW_NOM1)*coalesce(fut.M_QTY,1),
loan.M_REF_CAP0,
dlv.M_TOT_QTY,
case when trn.M_TRN_GTYPE in (77) then fxs.M_XPFWPRC else trn.M_BRW_RTE1 end, 
trn.M_BRW_MRG1,
trn.M_BRW_RTE2,
trn.M_BRW_MRG2,
trn.M_TRN_GTYPE,
...
qot.M_REFERENCE,
pub.M_REFERENCE, 
coalesce(lhsr.M_SERIE, ghsr.M_SERIE),
case  
