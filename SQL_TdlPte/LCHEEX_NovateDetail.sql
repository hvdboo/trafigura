...
-- (case when trn.M_COMMENT_BS ='B' then rtrim(trn.M_BTRADER) else rtrim(trn.M_STRADER) end) TRD,
...
rtrim(pub.M_LABEL) PUB_OFF, 'EEX FUTURE' PUB_TRF,
rtrim(plin.M_DSP_LABEL) INS_OFF,
case fut.M_LABEL
...
when 'FRT S TCA 10 - LCH' then 'FRT S TCA 10 - EEX' else null end INS_TRF,
rtrim(trn.M_BRW_ODPL) MAT,
to_char(fmat.M_QT_END,'YYYYMMDD') EXP,
to_char(trn.M_TRN_EXP,'YYYYMMDD') STL,
case rtrim(trn.M_COMMENT_BS)
when 'B' then 'Buy' 
when 'S' then 'Sell' else null end DIR,
trn.M_BRW_NOM1 NOM,
qotf.M_CURR||'/'||rtrim(uniq.M_LABEL) QOT,
case fut.M_LABEL
when 'FRT BCI - LCH' then to_char(hb9.M_P288,'999,999.9999')
when 'FRT BCI - SGX' then to_char(hb1.M_P128,'999,999.9999')
when 'FRT BPI - SGX' then to_char(hb2.M_P128,'999,999.9999')
when 'FRT BSI - LCH' then to_char(hb3.M_P288,'999,999.9999')
when 'FRT BSI - SGX' then to_char(hb4.M_P128,'999,999.9999')
when 'FRT C7 - LCH'  then to_char(hb5.M_P288,'999,999.9999')
when 'FRT CS TCA 5 - LCH' then to_char(hb6.M_P288,'999,999.9999')
when 'FRT CS TCA 5 - SGX' then to_char(hb7.M_P128,'999,999.9999')
when 'FRT S TCA 10 - LCH' then to_char(hb8.M_P288,'999,999.9999') else null end HISPRC,
to_char(trn.M_BRW_RTE1,'999,999.9999') PRC_ORI
/*
fut.M_REFERENCE FUTID, 
qotf.M_REFERENCE QOTID, 
rtrim(pub.M_LABEL) PUB,
pub.M_REFERENCE PUBID, 
rtrim(grp.M_HISFILE) HISID,
rtrim(hsr.M_LABEL) HSR,
hsr.M_SERIE HSRID,
fmat.M_REFERENCE MATID
*/
...
order by PFL, INS_OFF, to_char(trn.M_TRN_EXP,'YYYYMMDD'), rtrim(trn.M_COMMENT_BS)