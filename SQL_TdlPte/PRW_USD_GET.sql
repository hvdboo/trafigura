...
and trn.M_MOP_LAST not in (0,6,7)
and ext.M_CNL_STS = 1
and trn.M_TRN_EXP < PC.M_DATE
and trn.M_TRN_FMLY in ('COM','CURR')
and coalesce(ble.M_DSP_LABEL,sle.M_DSP_LABEL) in ('NTT','TNJ','TTS','TTY')
and trn.M_BPFOLIO not in ('MCEX APE PTEI','RMTS ZNPAM CMXD','SBOOK AP')
and trn.M_SPFOLIO not in ('MCEX APE PTEI','RMTS ZNPAM CMXD','SBOOK AP')
and trn.M_PL_INSCUR = 'USD'
and substr(trn.M_GID,1,3) not in ('BLN', 'PRW')
